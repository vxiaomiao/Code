...
f=c.rect();f.x=0;f.y=0;if(cc.rectContainsPoint(f,e))return c}return null}});_p=cc.Menu.prototype;cc.Menu.create=function(a){var b=arguments.length;0<b&&null==arguments[b-1]&&cc.log("parameters should not be ending with null in Javascript");return 0===b?new cc.Menu:1===b?new cc.Menu(a):new cc.Menu(Array.prototype.slice.call(arguments,0))};var res={HelloWorld_png:"res/HelloWorld.png",png_bg:"res/bg.jpg",png_1:"res/1.png",png_2:"res/2.png",png_3:"res/3.png",png_4:"res/4.png",png_5:"res/5.png",png_restart:"res/restart.png"},g_resources=[],i;for(i in res)g_resources.push(res[i]);var HelloWorldLayer=cc.Layer.extend({sprite:null,ctor:function(){this._super();var a=cc.winSize,b=new cc.LabelTTF("Hello World","Arial",38);b.x=a.width/2;b.y=a.height/2+200;this.addChild(b,5);this.sprite=new cc.Sprite(res.HelloWorld_png);this.sprite.attr({x:a.width/2,y:a.height/2});this.addChild(this.sprite,0);return!0}}),HelloWorldScene=cc.Scene.extend({onEnter:function(){this._super();var a=new HelloWorldLayer;this.addChild(a)}});var Candy=cc.Sprite.extend({_type:0,_column:0,_row:0,ctor:function(a,b,c){this._super(eval("res.png_"+(a+1).toString()));this.init(a,b,c)},init:function(a,b,c){this._type=a;this._column=b;this._row=c}});Candy.createRandomType=function(a,b){return new Candy(parseInt(Math.random()*GC.USERDATA.CANDY_TYPE_COUNT),a,b)};var GC=GC||{};GC.USERDATA={CANDY_TYPE_COUNT:5,CANDY_WIDTH:64,MAP_SIZE:10,FALL_ACCELERATION:30};GC.LEVELSDATA=[{limitStep:30,targetScore:500},{limitStep:25,targetScore:1E3},{limitStep:20,targetScore:2E3},{limitStep:20,targetScore:3E3},{limitStep:15,targetScore:4E3},{limitStep:10,targetScore:5E3}];var GameLayer=cc.Layer.extend({_isTouch:!0,_pointBefore:null,_ui:null,_mapPanel:null,_map:null,_level:0,_score:0,_limitStep:0,_steps:0,_targetScore:0,_restartBut:null,_moving:!1,ctor:function(){this._super();var a=cc.director.getWinSize(),b=new cc.Sprite(res.png_bg);b.attr({x:a.width/2,y:a.height/2});this.addChild(b,-1);b=new cc.MenuItemImage(res.png_restart,res.png_restart,res.png_restart,this.clickRestartButFunc,this);b.attr({x:a.width/2,y:200});b.setScale(2);this._restartBut=b;b=new cc.Menu(b);
b.attr({anchorX:0,anchorY:0,x:0,y:0});this.addChild(b,20);b=new cc.ClippingNode;this.addChild(b,2);this._mapPanel=new cc.Layer;this._mapPanel.x=(a.width-GC.USERDATA.CANDY_WIDTH*GC.USERDATA.MAP_SIZE)/2;this._mapPanel.y=(a.height-GC.USERDATA.CANDY_WIDTH*GC.USERDATA.MAP_SIZE)/2;b.addChild(this._mapPanel,1);a=new cc.DrawNode;a.drawRect(cc.p(this._mapPanel.x,this._mapPanel.y),cc.p(this._mapPanel.x+GC.USERDATA.CANDY_WIDTH*GC.USERDATA.MAP_SIZE,this._mapPanel.y+GC.USERDATA.CANDY_WIDTH*GC.USERDATA.MAP_SIZE),
cc.color(0,0,0),1,cc.p(0,0,0));b.stencil=a;cc.eventManager.addListener({event:cc.EventListener.TOUCH_ONE_BY_ONE,onTouchBegan:this.onTouchBegan.bind(this),onTouchEnded:this.onTouchEnded.bind(this)},this._mapPanel);this.init();this._ui=new GameUI(this);this.addChild(this._ui,3);return!0},clickRestartButFunc:function(){cc.log("clickRestartButFunc.");this.scheduleOnce(function(){this._steps=0;this._level=Storage.getCurrentLevel();this._score=Storage.getCurrentScore();this._limitStep=GC.LEVELSDATA[this._level].limitStep;
this._targetScore=GC.LEVELSDATA[this._level].targetScore;this.scheduleOnce(function(){cc.director.runScene(new GameScene)},0.3)},0.1)},init:function(){this._steps=0;this._level=Storage.getCurrentLevel();this._score=Storage.getCurrentScore();this._limitStep=GC.LEVELSDATA[this._level].limitStep;this._targetScore=GC.LEVELSDATA[this._level].targetScore;this._map=[];for(var a=0;a<GC.USERDATA.MAP_SIZE;a++){for(var b=[],c=0;c<GC.USERDATA.MAP_SIZE;c++){var d=Candy.createRandomType(a,c);this._mapPanel.addChild(d);
d.x=a*GC.USERDATA.CANDY_WIDTH+GC.USERDATA.CANDY_WIDTH/2;d.y=c*GC.USERDATA.CANDY_WIDTH+GC.USERDATA.CANDY_WIDTH/2;b.push(d)}this._map.push(b)}},onTouchBegan:function(a,b){cc.log("onTouchBegan");this._pointBefore=a.getLocation();return this._isTouch},onTouchEnded:function(a,b){cc.log("onTouchEnded.");var c=a.getLocation();if(this._pointBefore.x==c.x&&this._pointBefore.y==c.y){cc.log("The same point.");var d=Math.floor((c.x-this._mapPanel.x)/GC.USERDATA.CANDY_WIDTH),c=Math.floor((c.y-this._mapPanel.y)/
GC.USERDATA.CANDY_WIDTH);cc.log("("+d+","+c+")");0<=d&&9>=d&&0<=c&&9>=c?this.popCandy(d,c):cc.log("Other.")}else cc.log("Not same point.")},onMouseDown:function(a){cc.log("onMouseDown");var b=a.getLocationX();a=a.getLocationY();b=Math.floor((b-this._mapPanel.x)/GC.USERDATA.CANDY_WIDTH);a=Math.floor((a-this._mapPanel.y)/GC.USERDATA.CANDY_WIDTH);cc.log("("+b+","+a+")");0<=b&&9>=b&&0<=a&&9>=a?this.popCandy(b,a):cc.log("Other.")},popCandy:function(a,b){cc.log("popCandy_column_row \x3d ",a,b);if(!this._moving){var c=
[this._map[a][b]];cc.log("joinCandy \x3d ",c.length);for(var d=0,e=function(a){0>c.indexOf(a)&&(c.push(a),cc.log("pushIntoCandys_joinCandys.length \x3d ",c.length))};d<c.length;){var f=c[d];this.checkCandyExist(f._column-1,f._row)&&this._map[f._column-1][f._row]._type==f._type&&e(this._map[f._column-1][f._row]);this.checkCandyExist(f._column+1,f._row)&&this._map[f._column+1][f._row]._type==f._type&&e(this._map[f._column+1][f._row]);this.checkCandyExist(f._column,f._row-1)&&this._map[f._column][f._row-
1]._type==f._type&&e(this._map[f._column][f._row-1]);this.checkCandyExist(f._column,f._row+1)&&this._map[f._column][f._row+1]._type==f._type&&e(this._map[f._column][f._row+1]);d++}cc.log("joinCandys.length \x3d ",c.length);if(1>=c.length)cc.log("joinCandys.length \x3c\x3d1");else{this._steps++;this.moving=!0;for(d=0;d<c.length;d++)f=c[d],this._mapPanel.removeChild(f),this._map[f._column][f._row]=null;this._score+=c.length*c.length;this.generateNewCandy();this.checkSucceedOrFail()}}},checkCandyExist:function(a,
b){if(0<=a&&a<GC.USERDATA.MAP_SIZE&&0<=b&&b<GC.USERDATA.MAP_SIZE)return cc.log("in."),!0;cc.log("out.");return!1},generateNewCandy:function(){cc.log("generateNewCandy.");for(var a=0,b=0;b<GC.USERDATA.MAP_SIZE;b++){for(var c=0,d=0;d<this._map[b].length;d++){var e=this._map[b][d];if(e){var f=c;if(0<f){var g=Math.sqrt(2*f/GC.USERDATA.FALL_ACCELERATION);g>a&&(a=g);g=(new cc.MoveTo(g,e.x,e.y-GC.USERDATA.CANDY_WIDTH*f)).easing(cc.easeIn(2));e.runAction(g);e._row-=f;this._map[b][d]=null;this._map[b][e._row]=
e}}else e=Candy.createRandomType(b,GC.USERDATA.MAP_SIZE+c),e.x=e._column*GC.USERDATA.CANDY_WIDTH+GC.USERDATA.CANDY_WIDTH/2,e.y=e._row*GC.USERDATA.CANDY_WIDTH+GC.USERDATA.CANDY_WIDTH/2,this._mapPanel.addChild(e),this._map[b][e._row]=e,c++}for(d=this._map[b].length;d>=GC.USERDATA.MAP_SIZE;d--)this._map[b].splice(d,1)}this.scheduleOnce(this.finishCandyFalls.bind(this),a)},checkSucceedOrFail:function(){cc.log("this._score \x3d ",this._score);cc.log("this._targetScore \x3d ",this._targetScore);cc.log("this._steps \x3d ",
this._steps);cc.log("this._limitStep \x3d ",this._limitStep);this._score>=this._targetScore?(this._ui.showSuccess(),this._isTouch=!1,this._score+=30*(this._limitStep-this._steps),Storage.setCurrentLevel(this._level+1),Storage.setCurrentScore(this._score),this.scheduleOnce(function(){this._isTouch=!0;cc.director.runScene(new GameScene)},3)):this._steps>=this._limitStep&&(this._ui.showFail(),this._isTouch=!1,Storage.setCurrentLevel(0),Storage.setCurrentScore(0),this.scheduleOnce(function(){this._isTouch=
!0;cc.director.runScene(new GameScene)},3))},finishCandyFalls:function(){this.moving=!1}}),GameScene=cc.Scene.extend({onEnter:function(){this._super();var a=new GameLayer;this.addChild(a)}});var GameUI=cc.Layer.extend({_levelText:null,_scoreText:null,_stepText:null,_gameLayer:null,ctor:function(a){this._super();this._gameLayer=a;this.init();this.scheduleUpdate()},init:function(){Wsize=cc.director.getWinSize();var a=new cc.LabelTTF("Level","arial",36);a.attr({x:100,y:Wsize.height-50});a.setColor(cc.color(0,0,0));this.addChild(a);var b=new cc.LabelTTF("1","arial",36);b.attr({x:a.x,y:a.y-40});b.setColor(cc.color(0,0,0));this.addChild(b);this._levelText=b;var c=new cc.LabelTTF("Score","arial",
