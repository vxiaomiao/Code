...
var FoodManager = cc.Class.extend({
    ...
    ctor: function (gameScene) {
        this._container = gameScene._itemBatchLayer;
    ...
    update: function (hero, dt) {
        this.setFoodPattern(dt);
        this.createFoodPattern(dt);
        this.animateFoodItems(hero, dt);
    },

    setFoodPattern: function (dt) {
        // If hero has not travelled the required distance,
        // don't change the pattern.
        if (this._patternChangeDistance > 0) {
            this._patternChangeDistance -= Game.user.heroSpeed * dt;
        }
        else {
            // If hero has travelled the required distance,
            // change the pattern.
            if (Math.random() < 0.7) {
                // If random number is < normal item chance (0.7),
                // decide on a random pattern for items.
                this._pattern = Math.ceil(Math.random() * 4);
            }
            else {
                // If random number is > normal item chance (0.3),
                // decide on a random special item.
     ...
     */
    animateFoodItems: function (hero, dt) {
        var item;
        for (var i = this._itemsToAnimate.length - 1; i >= 0; i--) {
            ...
            if (item) {
                ...
                if (Game.user.mushroom > 0 && item._type <= GameConstants.ITEM_TYPE_5) {
                ...
                if (item.x < -80 || Game.gameState == GameConstants.GAME_STATE_OVER) {
                    ...
                    if (heroItem_sqDist < 5000) {
                        ...
                        if (item._type <= GameConstants.ITEM_TYPE_5) {
                            Game.user.score += item._type;
                            Sound.playEatEffect();
                        }
                        else if (item._type == GameConstants.ITEM_TYPE_COFFEE) {
                        ...
                        else if (item._type == GameConstants.ITEM_TYPE_MUSHROOM) {
